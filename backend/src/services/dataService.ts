...
import { escapeLike, sanitizeSearchQuery } from '../utils/sqlRelated.js';
import { calculatePagination } from '../utils/pagination.js';
import { RowSchema, validateCsvHeaders } from '../utils/schemaValidation.js';
    ...
    let successCount = 0;
        ...
        if (isFirstRow) {
            ...
            if (!headerValidation.isValid) {
                throw new Error(
                    `Invalid CSV structure. Missing columns: ${headerValidation.missing.join(', ')}`
                );
            }
            isFirstRow = false;
        }
...
export const fetchPaginatedComment = async (
    ...
    const cleanQuery = sanitizeSearchQuery(searchQuery); // truncate to prevent large search string that can slow server down
    ...
    const { safePage, safeLimit, offset, totalPages } = calculatePagination(page, limit, totalCount);
